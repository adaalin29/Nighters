...
@section('title', 'Produse - Nighters')
@section('content')
...
<div class = "filtre" id="desktop-filtre">
    ...
    <div class = "ordoneaza-container" id = "ordoneaza">
        ...
        <div class = "ordoneaza-element {{$discount_order != 'nada' ? 'ordoneaza-element-selected' : ''}}" onclick="put_to_order('discout','discount_order','discount');">Discount</div>
    </div>
    ...
    <div class = "ordoneaza-container" id = "filtreaza">
        ...
        <div class = "filtru-container">
            ...
            <div>
                ...
                <div class = "filtru-elemente">
                <?php
                    $lenjeri = $categorie->id == 5 ? true : false; 
                    
                    foreach($marimi as $marime){
                        if($lenjeri && strpos($marime->marime,'cm') !== false){
                            ?>
                            ...
                            <?php
                        } elseif(!$lenjeri && strpos($marime->marime,'cm') === false){
...
<div class = "container">
    <?php
        if(isset($categorie))
        {
            ...
            {
                ?>
                    <div class = "big-title" id="produse-big-title"><?php echo $categorie->nume;?></div>
                <?php
            }else{
                ?>
                    <div class = "small-title small-title-produs"><?php echo $categorie->nume;?></div>
                <?php
            }
            ?>
        ...
        {
            ?>
                <div class = "big-title" id="produse-big-title"><?php echo $subcategorie->name;?></div>
            <?php
        }
...
<!-- <div class = "produse-banner">
  <img src = "images/produs-banner.png" class = "full-width-cover">
</div> -->
...
<input type="hidden" id="slug_categorie" value="<?php echo $categorie->id;?>">

<?php
    if(isset($subcategorie))
    {
        ?>
        <input type="hidden" id="id_subcategorie" value="<?php echo $subcategorie->id;?>">
        <?php
    }
?>
...
<script>
    categorie = $('#slug_categorie').val();
    if( $('#id_subcategorie').length > 0){
        subcategorie =  $('#id_subcategorie').val();
        datasource2 = '/produsee/' + categorie + '/'+ subcategorie;
    }else{
        subcategorie = 'fara';
        datasource2 = '/produsee/' + categorie + '/' + subcategorie;
    }
    if($('#promotii_page').length > 0){
        datasource2 = datasource2+'?promotii=1';
    }
    ...
    function get_total_number(){
        $.ajax({
            ...
            success: function (resp) {
               get_data(resp.total_number);
            }
        });
    }
    

    function get_data(tt){
    ...
    function get_data_by_color(id_culoare){
        $tb_response = $('.produse-container');
        $targets = '';
        datasource = '/produsee/culoare?culoare=' + id_culoare + '&categorie=' + categorie;
        if( $('#id_subcategorie').length > 0){
            subcategorie =  $('#id_subcategorie').val();
            datasource = datasource + '&subcategorie=' + subcategorie;
        }
        $('#pagination_container').pagination({
            dataSource: datasource,
            pageSize: page_size,
    ...
    function ordoneaza(filtru){
        $tb_response = $('.produse-container');
        $targets = '';
        datasource = '/produsee/filtru?filtru=' + filtru +'&categorie=' + categorie;
        ...
        $.each(products, function(key, value)
        {
            $html += '<a href = "/produs/'+value.link+'" class = "produs">';
               ...
               if(value.promotie == 'da')
                  {
                      var procent = Math.round((value.pretvechi - value.pret)*100/value.pretvechi);
                  $html += '<div class = "produs-nou">-' + procent + '%</div>';
