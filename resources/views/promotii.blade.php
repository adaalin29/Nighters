...
@section('title','Promotii - Nighters')
...
<div class = "container">
    <div class = "big-title" id="produse-big-title">PROMOTII</div>   
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('.filtru-container-title-container').click(function(){
            if($(this).next('.filtru-elemente').hasClass('filtru-elemente-active')){
                $(this).next('.filtru-elemente').removeClass('filtru-elemente-active');
                $(this).next('.filtru-container-sageata').css('transform','rotate(180deg)');
            }else{
                $(this).next('.filtru-elemente').addClass('filtru-elemente-active');
                $(this).next('.filtru-container-sageata').css('transform','rotate(0deg)');
            }
        });
...
<script>
    ...
    var datasource = '/promotiii';
    var split_url = document.URL.split('?');
    if(split_url[1] !== undefined ){
        datasource = datasource + '?' + split_url[1];
    }
    ...
    function get_total_number(){
        $.ajax({
            ...
            success: function (resp) {
                get_promotii_data(resp.total_number);
            }
        });
    }

    function get_promotii_data(tt){
    ...
    function get_data_by_color(id_culoare){
        $tb_response = $('.produse-container');
        $targets = '';
        datasource = '/produsee/culoare?culoare=' + id_culoare +'&promotii=1';
    ...
    function ordoneaza(filtru){
        $tb_response = $('.produse-container');
        $targets = '';
        datasource = '/produsee/filtru?filtru=' + filtru +'&promotii=da';
